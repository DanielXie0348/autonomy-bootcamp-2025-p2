...
def read_queue(
    ...
    while not controller.is_exit_requested():
        try:
            ...
            main_logger.info(f"Received data from queue: {data}")
        except queue.Empty:
            continue
...
def main() -> int:
    ...
    # Create your queues
    telemetry_to_command = queue_proxy_wrapper.QueueProxyWrapper(manager)

    # Just set a timer to stop the worker after a while, since the worker infinite loops
    exit_timer_duration = TELEMETRY_PERIOD * NUM_TRIALS * 2 + NUM_FAILS
    threading.Timer(exit_timer_duration, stop, (controller,)).start()

    # Read the main queue (worker outputs)
    threading.Thread(
        target=read_queue, args=(telemetry_to_command, main_logger, controller)
    ).start()

    telemetry_worker.telemetry_worker(connection, controller, telemetry_to_command)
