...
def stop(
    ...
    """
    Request the worker to stop and log the reason.
    """
    local_logger.info("Stopping worker")
...
def read_queue(
    ...
    while not controller.is_exit_requested():
        try:
            output = output_queue.queue.get(timeout=1)  # Adjust timeout as needed
            main_logger.info(f"Output: {output}", True)
...
def main() -> int:
    ...
    threading.Timer(TELEMETRY_PERIOD * len(path), stop, (controller, main_logger)).start()
